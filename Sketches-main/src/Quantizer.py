...
def myFitter(
    ...
    suggestedDists = [None] * len(
        likelihoodTests)  # suggestedDists[i] will get the best-fit distributions accordingy to test i
...
def calcErr(
        ...
        weightDist: str = None,
        # distribution by which the MSE is weighted; when None, do not calculate the weighted MSE
    ...
    for i in range(len(orgVec)):
        if orgVec[i] == 0:
            continue
        weightedRelMseVec[idxInweightedRelMseVec] = scipy.stats.norm(0, stdev).pdf(orgVec[i]) * (
...
def quantize(
        ...
        clampOutliers: bool = False,
        # When True, clamp vec by removing all the values outside [lowerBnd, upperBnd], to remove outliers.
        lowerBnd: float = None,
        upperBnd: float = None,
...
def plotGrids(
    ...
    for i in range(len(resRecords)):
        ...
        curLine, = ax.plot(
            resRecord['grid'],
            np.array(range(lenGrid)),
            # Write the y index in reverse order, so that the legends' order will correspond the order of the plots.
        ...
        curLegend = ax.legend(handles=[curLine], bbox_to_anchor=(-0.24, i * (1.1 / len(resRecords)), 0., .102),
