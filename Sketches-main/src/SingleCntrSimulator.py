...
class SingleCntrSimulator(object):
    ...
    def __init__(
            ...
            seed=SEED,
            verbose=[]
            # defines which outputs would be written to .res / .pcl output files. See the VERBOSE macros as py.
    ):
    ...
    def runSingleCntrSingleModeWrEr(self):
        ...
        """
        self.cntrRecord['sumSqAbsEr'] = np.zeros(
            self.numOfExps)  # self.cntrRecord['sumSqAbsEr'][j] will hold the sum of the square absolute errors collected at experiment j.
        self.cntrRecord['sumSqRelEr'] = np.zeros(
            self.numOfExps)  # self.cntrRecord['sumSqRelEr'][j] will hold the sum of the square relative errors collected at experiment j.
        self.numOfPoints = np.zeros(
            self.numOfExps)  # self.numOfPoints[j] will hold the number of points collected for statistic at experiment j. The number of points varies, as it depends upon the random process of increasing the approximated cntr.
            ...
            while cntrVal < self.maxRealVal:
                ...
                if (self.cntrRecord['sampleProb'] == 1 or random.random() < self.cntrRecord[
                    ...
                    if (cntrNewVal != cntrVal):  # the counter was incremented
                        ...
                        if VERBOSE_LOG in self.verbose:
                            printf(self.log_file,
                                   'realValCntr={}, cntrVal={}, added sumSqEr={:.4f}\n'.format(realValCntr, cntrVal, (
                                               (realValCntr - cntrVal) / realValCntr) ** 2))
                    ...
                    if self.dwnSmple:
                        ...
                        if cntrValAfterInc == self.cntrRecord[
                            'cntr'].cntrMaxVal:  # the cntr reached its maximum values and no down-sample is used --> finish this experiment
                            break
    ...
    def runSingleCntrSingleModeRdEr(self):
        ...
        for rel_abs_n in [True, False]:
            ...
            for statType in ['Mse', 'normRmse']:
                dict = calcPostSimStat(
                    numMeausures=self.maxRealVal * np.ones(self.numOfExps),
                    # numMeausures[j] captures the # of points collected for statistic at experiment j. In other sim settings, this # may vary, as it depends upon the random process of increasing the approximated cntr.
                    sumSqEr=sumSqEr,
                    statType=statType,
                    verbose=self.verbose,
                    logFile=self.logFile,
        ...
        for self.cntrSize in cntrSizes:
            ...
            for self.mode in modes:
                ...
                if len(zeroEntries) > 0:
                    error(
                        f'mode={self.mode}: a zero entry in the divisor in entries\n{zeroEntries}.Divisor is\n{listOfVals[1:]}')
                points = {'X': listOfVals[:len(listOfVals) - 1],
                          'Y': np.divide(listOfVals[1:] - listOfVals[:-1], listOfVals[1:])}
    ...
    def measureResolutionsBySettingStrs(
        ...
        for settingStr in settingStrs:
            ...
            points = {'X': listOfVals[:len(listOfVals) - 1],
                      'Y': [(listOfVals[i + 1] - listOfVals[i]) / listOfVals[i + 1] for i in
                            range(len(listOfVals) - 1)]}
            if VERBOSE_PCL in self.verbose:
    ...
    def genCntrRecord(
        ...
        if self.mode.startswith('F2P') or self.mode.startswith('F3P'):
            ...
            if expSize == None:
                error(
                    'In SingleCntrSimulator.genCntrRecord(). For generating an FP.CntrMaster you must specify an expSize')
            self.cntrRecord = {'mode': 'FP',
                               'cntr': FP.CntrMaster(cntrSize=self.cntrSize, expSize=expSize, verbose=self.verbose)}
            ...
            self.cntrRecord = {'mode': self.mode,
                               'cntr': SEAD_stat.CntrMaster(cntrSize=self.cntrSize, expSize=self.expSize,
                                                            verbose=self.verbose)}
            ...
            self.cntrRecord = {'mode': self.mode,
                               'cntr': AEE.CntrMaster(cntrSize=self.cntrSize, cntrMaxVal=self.cntrMaxVal)}
        else:
    ...
    def runSingleCntrSingleMode(
            ...
            cntrMaxVal=None,
            # cntrMaxVal - The maximal value that the cntr can represent w/o down-sampling. When None (default), take cntrMaxVal from Confs.  global parameter (found in this file).
            expSize=None,
            # Size of the exponent. Relevant only for Static SEAD counter. If cntrMaxVal==None (default), take expSize from Confs global parameter (found in this file).
        ...
        if self.cntrRecord['cntr'].cntrMaxVal < self.maxRealVal and (not (self.dwnSmple)):
            warning(
                'The counter of type {}, cntrSize={}, hyperSize={}, can reach max val={} which is smaller than the requested maxRealVal {}, and no dwn smpling was used'.format(
                    self.cntrRecord['mode'], self.cntrSize, self.hyperSize, self.cntrRecord['cntr'].cntrMaxVal,
                    self.maxRealVal))
        ...
        print(
            'Started running runSingleCntr at t={}. erTypes={} mode={}, cntrSize={}, maxRealVal={}, cntrMaxVal={}'.format(
                datetime.now().strftime("%H:%M:%S"), self.erTypes, self.mode, self.cntrSize, self.maxRealVal,
                self.cntrRecord['cntr'].cntrMaxVal))
        ...
        for self.erType in self.erTypes:
            ...
            getattr(self,
                    f'runSingleCntrSingleMode{self.erType}')()  # Call the corresponding function, according to erType (read/write error, regular/RMSE).
...
def genCntrMasterFxp(
    ...
    if nSystem == 'F2P':
        ...
        if flavor == 'sr':
            ...
            if dwnSmpl:
                return F3P_li_ds.CntrMaster(cntrSize=cntrSize, numCntrs=numCntrs, hyperMaxSize=hyperSize,
                                            verbose=verbose)
            else:
...
def getAllValsFP(cntrSize=8,  # of bits in the cntr (WITHOUT the sign bit)
    ...
    myCntrMaster = FP.CntrMaster(cntrSize=cntrSize, expSize=expSize, verbose=verbose,
                                 signed=False)  # For efficiency, we generate an unsigned vec; later, we will take its mirror for the negative part.
...
def getAllCntrsMaxValsFxP():
    for nSystem in [
        'F2P']:  # ['F2P_si_h2', 'F2P_li_h2', 'F2P_si_h3', 'F3P_li_h3', 'F3P_si_h2', 'F3P_li_h2', 'F3P_si_h3', 'F3P_li_h3']:
        for flavor in ['si', 'li']:
            getCntrsMaxValsFxp(
                nSystem=nSystem,
                flavor=flavor,
