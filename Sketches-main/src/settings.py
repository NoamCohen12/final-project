...
traceInfo = [
    {'traceName': 'Caida1', 'traceFullName': 'Caida1_equinix-nyc.dirA.20181220-130000.UTC.anon', 'len': 25000000,
     'numFlows': 1801150, 'maxFlowSize': 455156},
    {'traceName': 'Caida2', 'traceFullName': 'Caida2_equinix-chicago.dirA.20160406-130000.UTC.anon', 'len': 25000000,
     'numFlows': 852129, 'maxFlowSize': 3128382},
]
...
confInterval = lambda ar, avg, confLvl=0.95: st.t.interval(confLvl, len(ar) - 1, loc=avg, scale=st.sem(ar)) if np.std(
    ar) > 0 else [avg, avg]
    ...
    for line in lines:
        ...
        if (mode not in [item['mode'] for item in list_of_dicts if item['mode'] == mode]):
            ...
            if len(splitted_line[1]) < 1:
                error('in settings.sortcntrMaxVals(). line={line}, splitted_line={splitted_line}')
            list_of_dicts.append({'mode': mode, 'cntrSize': int(mode.split('_n')[1].split('_')[0]),
                                  'maxVal': float(splitted_line[1].split('=')[1])})
