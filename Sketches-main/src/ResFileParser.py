...
class ResFileParser(object):
    ...
    def printAllPoints(self, cntrSize=None, cntrMaxVal=None, printToScreen=False):
        ...
        if (cntrSize == None and cntrMaxVal != None) or (cntrSize != None and cntrMaxVal == None):
            settings.error(
                'ResFileParser.printAllPoints() should be called with either cntrSize and cntrMaxVal having both default value, or both having non-default- values.')
        if cntrSize == None and cntrMaxVal == None:
            ...
            points = [point for point in self.points if
                      (point['cntrSize'] == cntrSize and point['cntrMaxVal'] == cntrMaxVal)]
    ...
    def genErVsCntrSizePlot(
        ...
        _, ax = plt.subplots()
        points = [point for point in self.points if
                  point['numOfExps'] == numOfExps and point['erType'].startswith(erType)]
        ...
        for mode in modes:
            ...
            if pointsOfThisMode == []:
                ...
                continue
            cntrSizes = [point['cntrSize'] for point in pointsOfThisMode if
                         (point['cntrSize'] >= minCntrSize and point['cntrSize'] <= maxCntrSize)]
            y = []
    ...
    def genErVsCntrSizeTable(
            ...
            maxValBy: str = None,
            normalizeByPerfectCntr: bool = True,
            # when True, normalize all mode's results by dividing them by the value obtained by a perfect counter
            normalizeByMinimal: bool = True,
            # when True, normalize all mode's results by dividing them by the value obtained by the lowest value at this row
    ):
        """
        ...
        points = [point for point in self.points if
                  point['numOfExps'] == numOfExps and point['statType'] == statType and point['rel_abs_n'] == rel_abs_n]
        if width != None:
            points = [point for point in points if point['width'] == width]

        if len(points) == 0:
            warning(
                f'No points found for numOfExps={numOfExps}, cntrSize={cntrSize}, statType={statType}, rel_abs_not={rel_abs_n}, width={width}')
            return
        ...
        for cntrSize in cntrSizes:
            ...
            if pointsOfThisCntrSizeErType == []:
                warning(
                    f'No points found for numOfExps={numOfExps}, cntrSize={cntrSize}, , statType={statType}, rel_abs_not={rel_abs_n}, erType={erType}, width={width}')
                continue
            ...
            if normalizeByPerfectCntr:
                ...
                if len(pointsOfPerfCntr) == 0:
                    error(
                        f'In ResFileParser.genErVsCntrSizeTable(). Requested normalizeByPerfectCntr, but no such points for cntrSize={cntrSize}')
            ...
            for mode in modes:
                ...
                if pointsToPrint == []:
                    warning(
                        f'In ResFilePerser.genErVsCntrSizeTable(). No points found for numOfExps={numOfExps}, cntrSize={cntrSize}, mode={mode}, width={width}, statType={statType}, rel_abs_n={rel_abs_n}')
                    if mode != modes[-1]:
                        printf(datOutputFile, ' & ')
                    continue
                if len(pointsToPrint) > 1:
                    warning(
                        f'found {len(pointsToPrint)} points for numOfExps={numOfExps}, cntrSize={cntrSize}, mode={mode}, width={width}, statType={statType}, rel_abs_n={rel_abs_n}')
    ...
    def genErVsMemSizePlotCms(
        ...
        if points == []:
            warning(
                f'In ResFileParser.genErVsMemSizePlotCms(). No points found for numOfExps={numOfExps}, cntrSize={cntrSize}, maxValBy={maxValByStr}, numIncs={numIncs}, statType={statType}, rel_abs_n={rel_abs_n}')
            return
        modes = [point['mode'] for point in points]
        modes = list(
            set([mode for mode in modes if mode not in ignoreModes]))  # uniquify and filter-out undesired modes
        ...
        for mode in modes:
            ...
            for width in widths:
                ...
                if len(pointsToPlot) > 1:
                    warning(
                        f'found {len(pointsToPlot)} points for numOfExps={numOfExps}, cntrSize={cntrSize}, mode={mode}, width={width}, statType={statType}, rel_abs_n={rel_abs_n}. Printing the duplicated points to ../res/debug.txt')
                    foundDups = True
            ...
            memSizesInKb = [depth * w * (cntrSize / 8) / KB for w in
                            widths]  # Memory size in KB = width * depth / 1024.
    ...
    def genErVsMemSizePlotSpaceSaving(
        ...
        for mode in modes:
            ...
            for memSize in memSizes:
                ...
                if len(pointsToPlot) > 1:
                    warning(
                        f'found {len(pointsToPlot)} points for numOfExps={numOfExps}, mode={mode}, memSize={memSize}, statType={statType}, rel_abs_n={rel_abs_n}. Printing the duplicated points to ../res/debug.txt')
                    foundDups = True
                    for point in pointsToPlot:
                        printfDict(dupsFile, point)
                point = pointsToPlot[0]
    ...
    def genErVsCntrMaxValPlot(
        ...
        for mode in [point['mode'] for point in self.points if point['mode'] in preferredModes]:
            pointsOfThisMode = [point for point in self.points if
                                (point['mode'] == mode and point['cntrSize'] == cntrSize)]
            ...
            for cntrMaxVal in [item for item in cntrMaxVals if (item >= 10000)]:
                ...
                pointOfThisModeNMaxVal = [point for point in pointOfThisModeNMaxVal if
                                          point['settingStr'] not in ['F2P_n8_h1', 'SEADstat_n8_e1']]  # $$$
                ...
                printf(datOutputFile,
                       'settingStr={}, mode={}. cntrMaxVal={}, y_lo={:.2f}, y_hi={:.2f}, y_avg={:.2f}\n'.format
    ...
    def genResolutionPlotByModes(self,
        ...
        by_label = dict(zip(labels, handles))
        plt.legend(by_label.values(), by_label.keys(), fontsize=LEGEND_FONT_SIZE, frameon=False)
        plt.savefig('../res/ResolutionByModes_n{}_{}.pdf'.format(cntrSize, 'log' if xLog else 'lin'),
                    bbox_inches='tight')
    ...
    def genResolutionPlotBySettingStrs(
        ...
        by_label = dict(zip(labels, handles))
        plt.legend(by_label.values(), by_label.keys(), fontsize=LEGEND_FONT_SIZE, frameon=False)
        plt.savefig('../res/resolutionBySettingStrs_n{}_{}.pdf'.format(cntrSize, 'log' if xLog else 'lin'),
                    bbox_inches='tight')

    def genErrByDfGraph(self,
                        cntrSize: int = 8,
                        resTypeStr: str = 'abs',
                        # a string detailing the y value for which the func' will generate a plot
        ...
        for mode in modes:
            pointsOfThisMode = [point for point in points if point['mode'] == mode]
            pointsOfThisMode = sorted(pointsOfThisMode,
                                      key=lambda item: item['df'])  # sort the points by their df value
    ...
    def ErrByDistBar(
        ...
        for i in range(len(modes)):
            mode = modes[i]
            if resTypeStr == 'rel' and mode.endswith(
                    '_h1'):  # for relative errors, 'h1' is always worse than 'h2', so we can skip them for the plot.
                continue
    ...
    def optModeOfDist(
        ...
        if len(points) == 0:
            print(
                f'In ResFileParser.optModeOfDist(). No points found for cntrSize={cntrSize}, errType={errType}, dist={distStr}')
            return None
    ...
    def printRndErrTableRow(
        ...
        for dist in dists:  # ['uniform', 'norm', 't_5', 't_8', 't_2', 't_4', 't_6', 't_10']: #'MobileNet_V2', 'MobileNet_V3', 'Resnet18', 'Resnet50']:#, 'uniform', 'norm', 't_5', 't_8', 't_2', 't_4', 't_6', 't_10']:
            ...
            if len(pointsOfThisDist) == 0:
                settings.error(
                    f'In ResFileParser.(). No points found for cntrSize={cntrSize}, errType={errType}, dist={dist}')
            ...
            for mode in modes:
                ...
                if len(pointsOfThisDistAndMode) == 0:
                    print(
                        f'In ResFileParser.optModeOfDist(). No points found for cntrSize={cntrSize}, errType={errType}, dist={dist}, mode={mode}')
                    printf(resFile, 'None & ')
                    continue
...
def genErrByDistBar():
    ...
    for cntrSize in [8]:  # , 16, 19]:
        ...
        for dist in ['uniform']:  # , 'Gaussian', 't_5', 't_8']:
            for resTypeStr in [
                'rel']:  # , 'abs']: # a string detailing the y value for which the func' will generate a plot
...
def printAllOptModes():
    ...
    for cntrSize in [8, 16, 19]:
        ...
        for distStr in dists:  # ['uniform', 'norm', 't_5', 't_8', 't_2', 't_4', 't_6', 't_10']: #'MobileNet_V2', 'MobileNet_V3', 'Resnet18', 'Resnet50']:#, 'uniform', 'norm', 't_5', 't_8', 't_2', 't_4', 't_6', 't_10']:
            bestF2PPoint = myResFileParser.optModeOfDist(cntrSize=cntrSize, distStr=distStr, errType=errType,
                                                         onlyF2P=True, onlyNonF2P=False)
            bestNonF2PPoint = myResFileParser.optModeOfDist(cntrSize=cntrSize, distStr=distStr, errType=errType,
                                                            onlyF2P=False, onlyNonF2P=True)
            if bestF2PPoint == None or bestNonF2PPoint == None:
                continue
            printf(resFile,
                   f'distStr={distStr}\t bestNonF2P={bestNonF2PPoint[0]}\t, bestNonF2PVal={bestNonF2PPoint[1]}\tbestF2PFlavor={bestF2PPoint[0]}\t, bestF2P/bestNonF2P={bestF2PPoint[1] / bestNonF2PPoint[1]}\n')
        if cntrSize in [8, 19]:
            continue
        elif cntrSize == 16:
            ignoreModes = ['FP_e10']
        ...
        for distStr in dists:  # ['uniform', 'norm', 't_5', 't_8', 't_2', 't_4', 't_6', 't_10']: #'MobileNet_V2', 'MobileNet_V3', 'Resnet18', 'Resnet50']:#, 'uniform', 'norm', 't_5', 't_8', 't_2', 't_4', 't_6', 't_10']:
            bestF2PPoint = myResFileParser.optModeOfDist(cntrSize=cntrSize, distStr=distStr, errType=errType,
                                                         onlyF2P=True, onlyNonF2P=False)
            bestNonF2PPoint = myResFileParser.optModeOfDist(cntrSize=cntrSize, distStr=distStr, errType=errType,
                                                            onlyF2P=False, onlyNonF2P=True, ignoreModes=ignoreModes)
            if bestF2PPoint == None or bestNonF2PPoint == None:
                continue
            printf(resFile,
                   f'distStr={distStr}\t bestNonF2P={bestNonF2PPoint[0]}\t, bestNonF2PVal={bestNonF2PPoint[1]}\tbestF2PFlavor={bestF2PPoint[0]}\t, bestF2P/bestNonF2P={bestF2PPoint[1] / bestNonF2PPoint[1]}\n')
